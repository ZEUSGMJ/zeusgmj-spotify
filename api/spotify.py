...
def darken(color, amount):
    ...
    # Calculate the amount to darken each channel
    darken_amount = int(255 * amount / 100)

    # Darken each channel
    r = max(0, r - darken_amount)
    g = max(0, g - darken_amount)
    b = max(0, b - darken_amount)

    # Convert the new RGB values back to hexadecimal notation
...
def lighten(color, amount):
    ...
    lighten_amount = int(255 * amount / 100)
...
def get_contrasting_colors(hex_color):
    ...
    if luminance > 0.5:
        mainColorHex = '000000'  # black
        mainColorRGB = (0, 0, 0)
        secondaryColorHex = lighten('#'+mainColorHex, 20)
    else:
        mainColorHex = 'FFFFFF'  # white
        mainColorRGB = (255, 255, 255)
        secondaryColorHex = darken('#'+mainColorHex, 20)
      

    return mainColorRGB, mainColorHex, secondaryColorHex
...
def makeSVG(data, background_color, border_color):
    ...
    if not "is_playing" in data:
        ...
        item = data["item"]
        listeningToList = ["Currently playing:", "Jamming to:", "Grooving to:", "Listening to:", "Bumping to:"]
        currentStatus = random.choice(listeningToList)

    if item["album"]["images"] == []:
        image = PLACEHOLDER_IMAGE
    else:
        image = loadImageB64(item["album"]["images"][1]["url"])
        image_response = requests.get(item["album"]["images"][1]["url"])
        image_data = image_response.content
        
        ct = colorthief.ColorThief(BytesIO(image_data))
        dominantColor = ct.get_color(quality=1)
        dominantHexColor = '{:02x}{:02x}{:02x}'.format(*dominantColor)
    ...
    if (dominantHexColor and background_color == "181414"):
        background_color = dominantHexColor

    if (dominantColor and (background_color == "181414" and border_color == "181414")):
        border_color = get_contrasting_colors(dominantHexColor)[1]

    song_color = get_contrasting_colors(background_color)[1]
    artist_color = get_contrasting_colors(background_color)[2]
    shadow_color = get_contrasting_colors(background_color)[0]
